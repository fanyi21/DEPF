...
scDHA <- function(dataname = dataname, original_label = original_label, data = data, k = NULL, method = "scDHA", sparse = FALSE, n = 5000, ncores = 10L, gen_fil = TRUE, do.clus = TRUE, sample.prob = NULL, seed = NULL) {
  ...
  if(nrow(data) >= 50000)
  {
    res <- scDHA.big(dataname = dataname, original_label = original_label, data = data, k = k, method = method, K = K, n = n, ncores = ncores, gen_fil = gen_fil, do.clus = do.clus, sample.prob = sample.prob, seed = seed)
  } else {
    res <- scDHA.small(dataname = dataname, original_label = original_label, data = data, k = k, method = method, K = K, n = n, ncores = ncores, gen_fil = gen_fil, do.clus = do.clus, sample.prob = sample.prob, seed = seed)
  }
  res
}
...
scDHA.small <- function(dataname = dataname, original_label = original_label, data = data, k = NULL, method = "scDHA", K = 3, n = 5000, ncores = 10L, gen_fil = TRUE, do.clus = TRUE, sample.prob = NULL, seed = NULL) {
  ...
  for (i in 1:9){
    cat("output 9 of ", i, " lantent: ", dataname, ".\n")
    filename <- paste(current.path,"/9_latent_data/",i, "/", dataname, i, ".mat", sep = "")
